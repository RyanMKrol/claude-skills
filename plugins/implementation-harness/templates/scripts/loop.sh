...
run_claude() {
  ...
  local raw="$LOOP_WT/.harness/worklog/.claude-out.jsonl"   # raw stream events — dashboard's live tail
  local out="$LOOP_WT/.harness/worklog/.claude-out"          # reassembled plain text — unchanged meaning
  local rc
  set +e
...
structural_checks() {
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r s; do
      [ -z "$s" ] && continue
      # Normalize glob-style scope entries (`src/foo/**`, `src/foo/*`, `src/foo/`) to a bare directory
      # prefix — else a scope authored with a trailing glob never matches its own files (all counted creep).
      s="${s%/}"; s="${s%/\*\*}"; s="${s%/\*}"
...
for ((i = 1; i <= MAX_ITERS; i++)); do
  ...
  while :; do
    ...
    rc=0; set +e; run_claude "$tmodel" "$teffort" "$(prompt "$task" "$branch")" || rc=$?; set -e
    if [ "$rc" = 10 ]; then
