...
4. **On "skip":** leave the `.example` in place (they can run `/implementation-harness-customize` again any
   time) and move on.
...
- Remind them: everything you wrote is under `.harness/custom/` and is **never touched by
  `/implementation-harness-upgrade`**; re-run `/implementation-harness-customize` anytime to set up more.
